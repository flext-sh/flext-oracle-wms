...
class TestsFlextOracleWmsAuthentication:
    ...
    def test_auth_method_exposes_wire_value(
        self, member: c.OracleWms.OracleWMSAuthMethod, expected_value: str
    ) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (m.OracleWms.AuthSettings(method=_Method.BASIC), "username and password"),
            (
    ...
    def test_validate_business_rules_reports_specific_failure(
        self, settings: p.OracleWms.AuthSettings, expected_fragment: str
    ) -> None:

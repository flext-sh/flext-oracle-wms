...
Exercises the PUBLIC contract of ``u.OracleWms.Client``:
...
class TestsFlextOracleWmsWmsClient:
    ...
    @classmethod
    def _stub_boundary(
        cls, monkeypatch: pytest.MonkeyPatch, outcome: p.Result[m.Api.HttpResponse]
    ) -> None:
        """Stub the external HTTP boundary FlextApi.request with an outcome."""

        def _request(
            _self: FlextApi, _request: m.Api.HttpRequest
    ...
    def test_explicit_settings_are_exposed_unchanged(self) -> None:
        ...
        client = u.OracleWms.Client(settings=settings)
    ...
    def test_get_boundary_failure_is_wrapped_with_context(
        ...
        self._stub_boundary(monkeypatch, r[m.Api.HttpResponse].fail("Network error"))
        result = client.get("/test-endpoint")
        tm.fail(result)

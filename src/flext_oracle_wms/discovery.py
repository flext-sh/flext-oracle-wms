"""Oracle WMS Discovery - Legacy discovery module for backward compatibility."""
...
from flext_oracle_wms.wms_operations import ApiClientProtocol
...
class DiscoveryStrategy:
    """Strategy for executing discovery steps."""
...
class EndpointDiscoveryStrategy(DiscoveryStrategy):
    """Strategy for discovering entities from endpoints."""
    ...
    async def execute_discovery_step(
        ...
        try:
            ...
            if api_result.is_failure:
                ...
                return FlextResult[bool].ok(DISCOVERY_FAILURE)
            ...
            return FlextResult[bool].ok(DISCOVERY_SUCCESS)
    ...
    async def _make_api_request(
        ...
        try:
            ...
            if result.data is None:
                return FlextResult[object].fail(f"No response data from {endpoint}")
            return FlextResult[object].ok(result.data)
        except Exception as e:
            return FlextResult[object].fail(f"Failed to call {endpoint}: {e}")
...
class FlextOracleWmsEntityDiscovery:
    ...
    def __init__(
        ...
        """Initialize entity discovery with API client and environment."""
        self.api_client = api_client
        self.environment = environment
        self.cache_manager: CacheManagerProtocol | dict[str, object] | None = (
            cache_manager
        )
    ...
    def discovery_endpoints(self) -> list[str]:
        """Get list of discovery endpoints."""
    ...
    async def discover_all_entities(
        ...
        """Discover all entities with optional patterns and caching."""
        ...
        if use_cache and isinstance(self.cache_manager, dict):
            ...
            if isinstance(cached, FlextOracleWmsDiscoveryResult):
                return FlextResult[FlextOracleWmsDiscoveryResult].ok(cached)
        ...
        return FlextResult[FlextOracleWmsDiscoveryResult].ok(perform.data)
    ...
    async def _perform_discovery(
        ...
        """Perform entity discovery with context and strategy."""
        start = time.time()
        ...
        if result.success and result.data is not None:
            ...
            return FlextResult[FlextOracleWmsDiscoveryResult].ok(rebuilt)
        return result
    ...
    def _apply_post_processing(
        ...
        """Apply post-processing to entities."""
    ...
    def _filter_entities(
        ...
        """Filter entities by include/exclude patterns."""
        filtered = entities
        if include_patterns:
            filtered = [
                e
    ...
    def _create_discovery_result(
        ...
        """Create discovery result with context."""
        try:
    ...
    async def _get_cached_discovery(
        ...
        return FlextResult[FlextOracleWmsDiscoveryResult].fail("Cache not implemented")
...
def flext_oracle_wms_create_cache_manager(
    ...
    """Create a cache manager for Oracle WMS operations."""
...
def flext_oracle_wms_create_entity_discovery(
    ...
    *,
    ...
    """Create an entity discovery instance for Oracle WMS."""

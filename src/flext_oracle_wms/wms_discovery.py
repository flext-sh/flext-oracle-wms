...
CacheValueDict = FlextOracleWmsTypes.Core.Dict
...
class FlextOracleWmsCacheConfig(FlextConfig):
    ...
    enable_cleanup: bool = Field(default=True, description="Enable cleanup")
    ...
    @classmethod
    def reset_global_instance(cls: type[FlextOracleWmsCacheConfig]) -> None:
        """Reset the global singleton cache instance."""
        if hasattr(cls, "_cache_global_instance"):
            cls._cache_global_instance = None

    def validate_business_rules(self: FlextOracleWmsCacheConfig) -> FlextResult[None]:
...
class FlextOracleWmsCacheEntry(FlextModels):
    ...
    def __post_init__(self: FlextOracleWmsCacheEntry) -> None:
    ...
    def is_expired(self: FlextOracleWmsCacheEntry) -> bool:
        """Check if cache entry has expired."""
        return (time.time() - self.timestamp) > self.ttl_seconds

    def validate_business_rules(self: FlextOracleWmsCacheEntry) -> FlextResult[None]:
    ...
    def is_valid(self: FlextOracleWmsCacheEntry) -> bool:
        """Check if cache entry is still valid."""
        return not self.is_expired()

    def update_access(self: FlextOracleWmsCacheEntry) -> FlextOracleWmsCacheEntry:
...
class FlextOracleWmsCacheStats(FlextModels):
    ...
    def validate_business_rules(self: FlextOracleWmsCacheStats) -> FlextResult[None]:
    ...
    def get_hit_ratio(self: FlextOracleWmsCacheStats) -> float:
    ...
    def update_hit(self: FlextOracleWmsCacheStats) -> FlextOracleWmsCacheStats:
    ...
    def update_miss(self: FlextOracleWmsCacheStats) -> FlextOracleWmsCacheStats:
...
class FlextOracleWmsCacheManager:
    ...
    def __init__(self, config: FlextOracleWmsCacheConfig) -> None:
        ...
        self._cache: dict[str, FlextOracleWmsCacheEntry] = {}
        # Backward-compatibility aliases expected by tests
        self._entity_cache: dict[str, FlextOracleWmsCacheEntry] = self._cache
        ...
        try:
            with self._lock:
                ...
                if entry is None:
                    ...
                    return FlextResult[object].fail(f"Cache miss for key: {key}")
                ...
                if entry.is_expired():
                    ...
                    return FlextResult[object].fail(f"Cache expired for key: {key}")
            ...
            return FlextResult[object].fail(f"Cache get failed: {e}")
    ...
    def set_entity(
        ...
        """
        result: FlextResult[object] = self.set(key, value, ttl_seconds)
        success_value = True
        return (
    ...
    def get_entity(self, key: str) -> FlextResult[object | None]:
        ...
        if result.success:
            ...
            return FlextResult[object | None].ok(result.value)
        # Convert miss or expired to success with None
        if result.error and (
            "Cache miss" in result.error or "Cache expired" in result.error
        ):
            return FlextResult[object | None].ok(None)
        return FlextResult[object | None].fail(result.error or "error")
    ...
    def set_schema(
        ...
        """
        return self.set_entity(key, value, ttl_seconds)

    def get_schema(self, key: str) -> FlextResult[object | None]:
    ...
    def get_metadata(self, key: str) -> FlextResult[object | None]:
    ...
    def get_statistics(self) -> FlextResult[FlextOracleWmsCacheStats]:
    ...
    def clear(self: FlextOracleWmsCacheManager) -> None:
    ...
    def invalidate_key(self, key: str) -> FlextResult[None]:
...
class FlextOracleWmsDynamicSchemaProcessor:
    ...
    def __init__(
        ...
        self.sample_size = (
            ...
            else FlextOracleWmsConstants.Pagination.DEFAULT_PAGE_SIZE
        )
    ...
    def process_records(
        ...
        try:
            if not records:
                return FlextResult[dict[str, FlextOracleWmsTypes.Core.Dict]].fail(
            ...
            return FlextResult[dict[str, FlextOracleWmsTypes.Core.Dict]].ok(schema)
    ...
    def discover_entity_schema(
        ...
        """
        return self.process_records(records, None)

    def process_entity_records(
        self,
        _entity_name: str,
        records: TOracleWmsRecordBatch,
        schema: dict[str, FlextOracleWmsTypes.Core.Dict] | None,
    ) -> FlextResult[TOracleWmsSchema]:
    ...
    def _calculate_schema_confidence(
        ...
        schema: dict[str, FlextOracleWmsTypes.Core.Dict],
    ) -> float:
...
class DiscoveryContext:
    ...
    include_patterns: FlextOracleWmsTypes.Core.StringList | None
    exclude_patterns: FlextOracleWmsTypes.Core.StringList | None
    all_entities: list[FlextOracleWmsEntity]
    errors: FlextOracleWmsTypes.Core.StringList
...
class FlextOracleWmsEntityDiscovery:
    ...
    def _get_cached_entity(self, key: str) -> FlextResult[FlextOracleWmsEntity]:
    ...
    def _parse_entities_response(
        ...
        entity_names_result: FlextResult[object] = parser.parse_entities_response(
            response_data
        )
    ...
    def discover_entities(
        self,
        include_patterns: FlextOracleWmsTypes.Core.StringList | None = None,
        exclude_patterns: FlextOracleWmsTypes.Core.StringList | None = None,
        *,
        use_cache: bool = True,
        ...
        try:
            ...
            if use_cache and self.cache_manager:
                cached_result: FlextResult[object] = self.cache_manager.get(cache_key)
                if cached_result.success and isinstance(cached_result.value, dict):
                    data = cached_result.value
                    raw_entities: list[FlextOracleWmsTypes.Core.Dict] = []
                    entities_data: FlextOracleWmsTypes.Core.List = []
                    ...
                    entities: list[FlextOracleWmsEntity] = [
                        FlextOracleWmsEntity(
                            ...
                            fields=cast(
                                "FlextOracleWmsTypes.Core.Dict", item.get("fields")
                            )
            ...
            for strategy in self.strategies:
                _ = strategy.execute_discovery_step(context, self.api_client)
            ...
            if use_cache and self.cache_manager:
                ...
                self.cache_manager.set(cache_key, str(cache_data))
    ...
    def discover_entity_schema(
        ...
        try:
            ...
            discovery_result: FlextResult[object] = self.discover_entities()
    ...
    def _deduplicate_entities(
        ...
        seen_names = set()
        unique_entities: list[FlextOracleWmsEntity] = []
...
class EndpointDiscoveryStrategy(DiscoveryStrategy):
    ...
    def _make_api_request(
        ...
        try:
            ...
            result: FlextResult[object] = api_client.get(endpoint)
            ...
            response_data: FlextOracleWmsTypes.Core.Dict = result.unwrap()
    ...
    def execute_discovery_step(
        ...
        try:
            ...
            if hasattr(api_client, "get"):
                ...
                result: FlextResult[object] = api_client.get(

...
Enterprise Oracle WMS (Warehouse Management System) REST client library. Base for
`flext-tap-oracle-wms`, `flext-target-oracle-wms`, `flext-dbt-oracle-wms`.
...
- Client creation resolves base URL, timeout, headers, and SSL verification **from
  settings** — never embed transport defaults inline.
...
- Go through `FlextOracleWmsApi`; the client is a private `_client`, not a public
  surface.
